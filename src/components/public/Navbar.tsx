      ...
      <div className="container-custom">
        <div className="grid grid-cols-[auto_1fr_auto] items-center h-16 md:h-[72px]">
          ...
          <div className="flex lg:hidden items-center gap-1 justify-end">
            <button onClick={() => setIsSearchOpen(v => !v)} className={`flex items-center justify-center w-10 h-10 rounded-xl transition-all ${isSearchOpen ? 'bg-primary/10 text-primary' : `${isScrolled ? 'text-text hover:bg-surface-muted' : 'text-white/80 hover:bg-white/10'}`}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {isSearchOpen ? (
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                ) : (
            ...
            <button onClick={() => setIsMobileMenuOpen(v => !v)} className={`p-2 rounded-xl transition-all ${isMobileMenuOpen ? 'bg-primary/10 text-primary' : `${isScrolled ? 'text-text hover:bg-surface-muted' : 'text-white hover:bg-white/10'}`}`}>
